...
bg = tk.PhotoImage("logo5.png")
label1 = Label( root, image = bg)
label1.place(x = 0, y = 0)

def convertimg():
    img = Image.open("logo1.jpg")  # Allows us to open CSUMB logo image #
    img = img.convert("RGBA")  # Converts image to an RGBA format #
    ...
    for item in datas:
       if item[:3] == (255, 255, 255): # Pulls the white RGB value from the CSUMB logo #
          newData.append((0, 0, 0, 0)) # Then replaces the white with transparency/red RGB value #
       else:
          newData.append(item)
    img.putdata(newData)  # Applies changes to the image
    img.save("logotrans.png")
